...
ENGINE_URL = "http://127.0.0.1:5050/inject"  # was 5000 

# Network interface to capture on (None = default interface)
# Examples: "eth0", "wlan0", "en0"
# Network interface to capture on.
# Set to None to use the system default, or specify your interface name.
# Examples: "eth0" (Linux), "en0" (macOS Wi-Fi), "wlan0" (Linux Wi-Fi)
# Find yours with: ip a  (Linux) or  ifconfig  (macOS)
INTERFACE = None #MacOs= en0 , 
...
def start():
    ...
    # Start the flow flusher thread — guarded so it is never started twice
    if not _flusher_started:
        _flusher_started = True
        t = threading.Thread(target=flush_flows, daemon=True, name="flusher")
        t.start()
        print(f"[OK]  Flow flusher thread started (tid={t.ident})")
