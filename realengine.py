...
SNIFF_INTERFACE       = "en0"           # active macOS interface
...
# Fill in your credentials in a .env file — NEVER hardcode passwords here.
# Email (Gmail):
#   NOTIFY_EMAIL_FROM=you@gmail.com
#   NOTIFY_EMAIL_TO=you@gmail.com
#   NOTIFY_EMAIL_PASS=your_app_password   ← Gmail App Password (not your real password)
# Telegram:
#   NOTIFY_TELEGRAM_TOKEN=123456:ABC...
#   NOTIFY_TELEGRAM_CHAT_ID=987654321
# Set to "1" to enable each channel:
#   NOTIFY_EMAIL=1
#   NOTIFY_TELEGRAM=1

NOTIFY_EMAIL_ENABLED    = os.getenv("NOTIFY_EMAIL",    "0") == "1"
NOTIFY_TELEGRAM_ENABLED = os.getenv("NOTIFY_TELEGRAM", "0") == "1"
...
# Minimum seconds between notifications for the same IP (prevents spam)
NOTIFY_COOLDOWN = int(os.getenv("NOTIFY_COOLDOWN", "120"))
_notify_times: dict = {}   # ip → last notification epoch
_notify_lock = threading.Lock()

def _should_notify(ip: str) -> bool:
    now = time.time()
    with _notify_lock:
        if now - _notify_times.get(ip, 0) < NOTIFY_COOLDOWN:
            return False
        _notify_times[ip] = now
        return True

def send_notification(subject: str, body: str, ip: str):
    """Fire-and-forget notification via email and/or Telegram."""
    if not _should_notify(ip):
        return
    def _send():
        # ── Email ──────────────────────────────────────────────────────────────
        if NOTIFY_EMAIL_ENABLED and NOTIFY_EMAIL_FROM and NOTIFY_EMAIL_PASS:
            try:
                import smtplib
                from email.mime.text import MIMEText # MIMEText used for email body 
                msg = MIMEText(body)
                msg["Subject"] = subject
                msg["From"]    = NOTIFY_EMAIL_FROM
                msg["To"]      = NOTIFY_EMAIL_TO
                with smtplib.SMTP_SSL("smtp.gmail.com", 465) as s:
                    s.login(NOTIFY_EMAIL_FROM, NOTIFY_EMAIL_PASS)
                    s.send_message(msg)
                print(f"[NOTIFY] Email sent for {ip}")
            except Exception as e:
                print(f"[NOTIFY ERR] Email: {e}")
        # ── Telegram ───────────────────────────────────────────────────────────
        if NOTIFY_TELEGRAM_ENABLED and NOTIFY_TELEGRAM_TOKEN and NOTIFY_TELEGRAM_CHAT_ID:
            try:
                text = f"*{subject}*\n\n{body}"
                requests.post(
                    f"https://api.telegram.org/bot{NOTIFY_TELEGRAM_TOKEN}/sendMessage",
                    json={"chat_id": NOTIFY_TELEGRAM_CHAT_ID,
                          "text": text, "parse_mode": "Markdown"},
                    timeout=5
                )
                print(f"[NOTIFY] Telegram sent for {ip}")
            except Exception as e:
                print(f"[NOTIFY ERR] Telegram: {e}")
...
with db_lock:
    ...
    # ── NEW: Deduplicated incidents ───────────────────────────────────────────
    c.execute("""CREATE TABLE IF NOT EXISTS incidents (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        source_ip TEXT, attack_type TEXT, severity TEXT,
        risk_score INTEGER, hit_count INTEGER DEFAULT 1,
        first_seen TEXT, last_seen TEXT,
        lat REAL, lon REAL, extra TEXT,
        UNIQUE(source_ip, attack_type)
    )""")
    # ── NEW: Risk score history ───────────────────────────────────────────────
    c.execute("""CREATE TABLE IF NOT EXISTS risk_history (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        timestamp TEXT, source_ip TEXT,
        attack_type TEXT, risk_score INTEGER
    )""")
...
_self_loc_lock = threading.Lock()
SELF_LAT, SELF_LON = 0.0, 0.0

def _fetch_self_location():
    """Fetch this machine's public lat/lon from ipinfo.io.
    Called at startup and retried lazily on first LAN IP lookup if startup failed."""
    global SELF_LAT, SELF_LON
    try:
...
# Try at startup; failure is non-fatal — will retry on first LAN IP hit.
if not _fetch_self_location():
    print("[WARN] ipinfo.io unreachable at startup — self-location set to 0,0. "
          "Will retry on first private-IP geo lookup.")
...
def geolocate(ip: str) -> dict:
    global SELF_LAT, SELF_LON
    if _is_private(ip):
        # Retry self-location lazily if startup failed (SELF_LAT/LON still 0,0)
        if SELF_LAT == 0.0 and SELF_LON == 0.0:
            with _self_loc_lock:
                if SELF_LAT == 0.0 and SELF_LON == 0.0:
                    _fetch_self_location()
...
# Synthetic training set — 60 rows covering normal and attack-like traffic patterns.
# Columns: packet_count, duration, port_count
# Normal traffic: low packet counts, few ports, standard durations
# Attack-like: high packet counts (DDoS), many ports (scans), or unusual combos
_training_data = pd.DataFrame([
    # ── Normal traffic (diverse but benign) ───────────────────────────────────
    [8,  5, 1], [12, 5, 1], [15, 5, 2], [20, 5, 1], [10, 5, 1],
    [18, 5, 3], [25, 5, 2], [30, 5, 1], [22, 5, 2], [14, 5, 1],
    [9,  5, 1], [11, 5, 1], [16, 5, 2], [19, 5, 1], [13, 5, 2],
    [28, 5, 3], [24, 5, 2], [17, 5, 1], [21, 5, 1], [26, 5, 2],
    [6,  5, 1], [7,  5, 1], [23, 5, 2], [27, 5, 1], [31, 5, 1],
    # ── Slightly elevated (bulk transfer, video stream) ───────────────────────
    [120, 5, 1], [200, 5, 1], [180, 5, 2], [150, 5, 1], [95,  5, 1],
    [110, 5, 1], [130, 5, 2], [160, 5, 1], [175, 5, 1], [145, 5, 2],
    # ── Port scans (many distinct ports, low-to-mid packet count) ─────────────
    [40,  5, 15], [50,  5, 20], [60,  5, 30], [35,  5, 12], [55,  5, 25],
    [45,  5, 18], [70,  5, 35], [80,  5, 40], [65,  5, 28], [75,  5, 38],
    # ── DDoS-like (very high packet counts, few ports) ────────────────────────
    [1200, 5, 1], [2000, 5, 1], [3000, 5, 2], [1500, 5, 1], [800,  5, 1],
    [1800, 5, 2], [2500, 5, 1], [900,  5, 1], [1100, 5, 1], [4000, 5, 1],
    # ── Mixed anomalies (high packets AND many ports) ─────────────────────────
    [500,  5, 50], [700,  5, 60], [600,  5, 45], [750,  5, 55], [850,  5, 70],
    [400,  5, 40], [650,  5, 52], [550,  5, 48], [450,  5, 42], [300,  5, 35],
], columns=["packet_count", "duration", "port_count"])

_model.fit(_training_data)
print(f"[OK]  IsolationForest model trained on {len(_training_data)} samples")
...
def analyse_flow(flow: dict):
    ...
    extra    = {"isp":geo.get("isp",""),"asn":geo.get("asn",""),
                ...
                "dst_port":dst_port}  # stored so the dashboard can chart targeted ports
    ...
    with db_lock:
        ...
        # ── 1. Raw alert log (unchanged — full history) ───────────────────────
        cur.execute("""
            INSERT INTO alerts
        ...
        # ── 2. Deduplicated incident (upsert) ─────────────────────────────────
        # If same IP + attack_type seen before → increment hit_count + update last_seen
        # Otherwise → create a new incident row
        cur.execute("""
            INSERT INTO incidents
              (source_ip,attack_type,severity,risk_score,hit_count,
               first_seen,last_seen,lat,lon,extra)
            VALUES (?,?,?,?,1,?,?,?,?,?)
            ON CONFLICT(source_ip,attack_type) DO UPDATE SET
              hit_count  = hit_count + 1,
              last_seen  = excluded.last_seen,
              risk_score = MAX(risk_score, excluded.risk_score),
              severity   = excluded.severity,
              extra      = excluded.extra
        """, (src,attack_type,severity,risk_score,ts,ts,lat,lon,extra_json))

        # ── 3. Risk score history snapshot ───────────────────────────────────
        cur.execute("""
            INSERT INTO risk_history(timestamp,source_ip,attack_type,risk_score)
            VALUES(?,?,?,?)
        """, (ts, src, attack_type, risk_score))

        conn.commit()

    # ── 4. Auto-block + notification for High severity ────────────────────────
    if severity == "High":
        with db_lock:
            cur = conn.cursor()
            try:
        ...
        geo_str  = extra.get("country","") or "Unknown"
        isp_str  = extra.get("isp","") or "Unknown"
        subject  = f"🚨 AegisNet HIGH ALERT — {attack_type} from {src}"
        body     = (
        ...
        send_notification(subject, body, src)
...
def status():
    return jsonify({
        "engine":"realengine.py v3","iptables":IPTABLES_ENABLED,
        ...
        "notify_email":NOTIFY_EMAIL_ENABLED,
        "notify_telegram":NOTIFY_TELEGRAM_ENABLED,
    })
...
def incidents():
    """Returns one row per (source_ip, attack_type) with hit_count."""
    lim = int(request.args.get("limit", 200))
    ip  = request.args.get("ip", "")
    with db_lock:
        cur = conn.cursor()
        if ip:
            cur.execute(
                "SELECT * FROM incidents WHERE source_ip=?"
                " ORDER BY last_seen DESC LIMIT ?", (ip, lim))
        else:
            cur.execute(
                "SELECT * FROM incidents ORDER BY last_seen DESC LIMIT ?", (lim,))
        cols = [d[0] for d in cur.description]
        return jsonify([dict(zip(cols, r)) for r in cur.fetchall()])
...
def risk_history():
    """Returns risk score snapshots for one IP or all IPs."""
    ip  = request.args.get("ip", "")
    ...
    with db_lock:
        ...
        if ip:
            cur.execute(
                "SELECT * FROM risk_history WHERE source_ip=?"
                " ORDER BY id ASC LIMIT ?", (ip, lim))
        else:
            cur.execute(
                "SELECT * FROM risk_history ORDER BY id ASC LIMIT ?", (lim,))
        cols = [d[0] for d in cur.description]
        return jsonify([dict(zip(cols, r)) for r in cur.fetchall()])
...
def test_notify():
    ...
    send_notification(
        subject="🛡️ AegisNet — Test Notification",
        body="This is a test notification from AegisNet.\nYour alert channels are working correctly.",
        ip="test"
    )
    channels = []
    if NOTIFY_EMAIL_ENABLED:    channels.append("email")
    if NOTIFY_TELEGRAM_ENABLED: channels.append("telegram")
    if not channels:
        return jsonify({"status": "no_channels",
                        "msg": "No notification channels enabled. Set NOTIFY_EMAIL=1 or NOTIFY_TELEGRAM=1 in your .env file."})
    return jsonify({"status": "sent", "channels": channels})
...
if __name__ == "__main__":
    try:
        ...
        # ARP scan requires root but NOT iptables — run it on macOS too.
        if os.geteuid() == 0:
            threading.Thread(target=arp_scan, daemon=True, name="arp").start()
        else:
            print("[ARP]  Skipped (not root — restart with sudo)")
        threading.Thread(
...
# NOTE: Store your GeoIP credentials in a .env file — never hardcode passwords in source.
# Run: geoipupdate -f GeoIP.conf -d geoip_db/  to refresh the MaxMind database.
